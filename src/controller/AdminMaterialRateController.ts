  ...
  async getMaterialLatestRate(req: Request, res: Response) {
    ...
    if (missingFields.length > 0) {
      ...
      try {
        ...
        logger.error('getLatestRate - Error fetching latest rate.', error);
        responseData = prepareJSONResponse({}, 'Error Exception.', statusCodes.INTERNAL_SERVER_ERROR);
      }
    }
    logger.info(`getLatestRate - Req and Res: ${JSON.stringify(requestBody)} - ${JSON.stringify(responseData)}`);
    return res.status(responseData.status).json(responseData);
  }
