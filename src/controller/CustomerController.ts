...
export default class CustomersController {
  ...
  async createDigitalPurchase(req: Request, res: Response) {
    const requestBody = req.body;
    const mandatoryFields = ['customer_id', 'material_id', 'amount'];
    const missingFields = mandatoryFields.filter((field) => !requestBody[field]);
    let responseData: typeof prepareJSONResponse = {};
    let message = 'Missing required fields';
    ...
    if (missingFields.length > 0) {
      ...
      try {
        ...
        if (!recordExists) {
          ...
          const material_id = requestBody.material_id;
          const amountNum = parseFloat(String(requestBody.amount));
          const price_per_gram_num = 11000;
          const tax_percentage_num = 3;

          if (Number.isNaN(amountNum) || amountNum <= 0) {
            responseData = prepareJSONResponse({}, 'Invalid amount', statusCodes.BAD_REQUEST);
          } else if (amountNum < 100) {
            responseData = prepareJSONResponse({}, 'Minimum purchase amount should be ₹100', statusCodes.BAD_REQUEST);
          } else {
            const grams_purchased_num = amountNum / price_per_gram_num;
            const grams_purchased = grams_purchased_num.toFixed(6);

            const tax_amount_num = (amountNum * tax_percentage_num) / 100;
            const tax_amount = Number(tax_amount_num.toFixed(2));

            let convenience_fee_rate = 0;
            if (amountNum < 1000) convenience_fee_rate = 6;
            else if (amountNum >= 1000 && amountNum < 5000) convenience_fee_rate = 4.5;
            else convenience_fee_rate = 3;

            const convenience_fee_num = (amountNum * convenience_fee_rate) / 100;
            const convenience_fee = Number(convenience_fee_num.toFixed(2));
            const total_amount_num = amountNum + tax_amount + convenience_fee;
            ...
            const purchaseData = {
              ...
              price_per_gram: price_per_gram_num,
              grams_purchased,
              tax_percentage: tax_percentage_num.toFixed(2),
              tax_amount,
            ...
            const newPurchase = await this.digitalPurchaseModel.create(purchaseData);
